...
function loadInitialMessages(groupId) {
    ...
    axios.get(`http://13.235.112.144/chat/msg?groupId=${groupId}&lastId=${lastMessageIds[groupId]}`, {
...
function handleFormSubmit(event) {
    ...
    const token = localStorage.getItem('token');
    axios.post('http://13.235.112.144/chat/msg', obj, {
        headers: { 'Authorization': token }
    }).then((res) => {
        console.log(res);
...
function handleGetChat() {
    ...
    const token = localStorage.getItem('token');
    axios.get(`http://13.235.112.144/chat/msg?groupId=${currentGroupId}&lastId=${lastMessageIds[currentGroupId]}`, {
        headers: { 'Authorization': token }
    }).then((res) => {
...
function fetchGroupMembers(groupId) {
    const token = localStorage.getItem('token');
    axios.get(`http://13.235.112.144/group/${groupId}/members`, {
        headers: { 'Authorization': token }
    }).then((res) => {
...
function searchUsers(query) {
    const token = localStorage.getItem('token');
    axios.get(`http://13.235.112.144/group/search-users?query=${query}&groupId=${currentGroupId}`, {
        headers: { 'Authorization': token }
    }).then((res) => {
...
function addMemberToGroup(groupId, userId) {
    const token = localStorage.getItem('token');
    axios.post('http://13.235.112.144/group/add-member', 
        { groupId, userId },
        { headers: { 'Authorization': token } }
    ).then(() => {
...
function createGroup() {
    ...
    if (groupName) {
        const token = localStorage.getItem('token');
        axios.post('http://13.235.112.144/group/create', { name: groupName }, {
            headers: { 'Authorization': token }
        }).then(() => {
            getUserGroups();
        }).catch(console.error);
...
function makeAdmin(userId) {
    ...
    const token = localStorage.getItem('token');
    axios.post('http://13.235.112.144/group/make-admin', { groupId: currentGroupId, userId }, {
        headers: { 'Authorization': token }
    }).then(() => {
...
function removeUser(userId) {
    ...
    if (confirm("Are you sure you want to remove this user from the group?")) {
        const token = localStorage.getItem('token');
        axios.post('http://13.235.112.144/group/remove-user', { groupId: currentGroupId, userId }, {
            headers: { 'Authorization': token }
        }).then(() => {
...
function getUserGroups() {
    const token = localStorage.getItem('token');
    axios.get('http://13.235.112.144/group/user-groups', {
        headers: { 'Authorization': token }
    }).then((res) => {
